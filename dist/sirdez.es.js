...
const struct = (definition) => {
  ...
  return /* @__PURE__ */ new Function("d", `[${indexes.map((i) => "k" + i)}]`, `[${indexes.map((i) => "s" + i)}]`, `[${indexes.map((i) => "d" + i)}]`, `return d((c,d)=>{${indexes.map((i) => `s${i}(c,d[${nameOf(keys[i])}])`).join(";")}},(c)=>{const d=${definition instanceof Array ? "[]" : "{}"};${indexes.map((i) => `d[${nameOf(keys[i])}]=d${i}(c)`).join(";")};return d})`)(define, keys, /* @__PURE__ */ values.map(({ ser }) => ser), /* @__PURE__ */ values.map(({ des }) => des));
};
...
const number = (kind, bitSize) => {
  ...
  return /* @__PURE__ */ new Function("d", `return d((c,d)=>{c.view.set${name}(c.i,d);c.i+=${size2}},(c)=>{const d=c.view.get${name}(c.i);c.i+=${size2};return d})`)(define);
};
...
const size = define((ctx, data) => usize.ser(ctx, data >= 0 ? data * 2 : data * -2 - 1), (ctx) => {
...
export { array, bigInt64, bigUint64, boolean, bytes, contextDes, contextFromBytes, contextSer, createContext, define, float32, float64, growContext, int16, int32, int8, latin1, map, number, optional, size, string, struct, tuple, ucs2, uint16, uint32, uint8, use, usize, utf8, utf8js };

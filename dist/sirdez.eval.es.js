...
const uint8$1 = define(
  (ctx, data) => ctx.view.setUint8(ctx.i++, data),
  (ctx) => ctx.view.getUint8(ctx.i++)
);
const uint16$1 = define(
  (ctx, data) => {
...
function packSize(ctx, size2, a, b) {
  if (size2 < 16) {
    uint8$1.ser(ctx, a + size2);
  } else if (size2 < 65536) {
    uint8$1.ser(ctx, b);
    uint16$1.ser(ctx, size2);
  } else {
    uint8$1.ser(ctx, b + 1);
    uint32$1.ser(ctx, size2);
  }
}
const strSize = {
  ser(ctx, size2) {
    if (size2 < 32) {
      uint8$1.ser(ctx, 160 + size2);
    } else if (size2 < 256) {
      uint8$1.ser(ctx, 217);
      uint8$1.ser(ctx, size2);
    } else if (size2 < 65536) {
      uint8$1.ser(ctx, 218);
      uint16$1.ser(ctx, size2);
    } else {
...
function unpack(ctx, encoding) {
  ...
  switch (byte) {
    ...
    case 195:
      return true;
    case 202:
      return float32$1.des(ctx);
    case 203:
      return float64$1.des(ctx);
    case 204:
      return uint8$1.des(ctx);
    case 205:
      return uint16$1.des(ctx);
    case 206:
      return uint32$1.des(ctx);
    case 207:
      return Number(bigUint64$1.des(ctx));
    case 208:
      return int8$1.des(ctx);
    case 209:
      return int16$1.des(ctx);
    case 210:
      return int32$1.des(ctx);
    case 211:
      return Number(bigInt64$1.des(ctx));
    case 217:
      return encoding.decode(ctx, uint8$1.des(ctx));
    case 218:
      return encoding.decode(ctx, uint16$1.des(ctx));
    case 219:
      return encoding.decode(ctx, uint32$1.des(ctx));
    case 220:
      return unpackArrayBody(ctx, uint16$1.des(ctx), encoding);
    case 221:
      return unpackArrayBody(ctx, uint32$1.des(ctx), encoding);
    case 222:
      return unpackMapBody(ctx, uint16$1.des(ctx), encoding);
    case 223:
      return unpackMapBody(ctx, uint32$1.des(ctx), encoding);
  }
  if (byte < 128)
    return byte;
  if (byte < 144)
...
const packInt = (ctx, data) => {
  if (data >= 0) {
    if (data < 128) {
      uint8$1.ser(ctx, data);
    } else if (data < 256) {
      uint8$1.ser(ctx, 204);
      uint8$1.ser(ctx, data);
    } else if (data < 65536) {
      uint8$1.ser(ctx, 205);
      uint16$1.ser(ctx, data);
    } else if (data < 4294967296) {
      uint8$1.ser(ctx, 206);
      uint32$1.ser(ctx, data);
    } else {
      uint8$1.ser(ctx, 207);
      bigUint64$1.ser(ctx, BigInt(data));
    }
  } else {
    if (data >= -32) {
      int8$1.ser(ctx, data);
      ...
      uint8$1.ser(ctx, 209);
      int16$1.ser(ctx, data);
    } else if (data >= -2147483648) {
      uint8$1.ser(ctx, 210);
      int32$1.ser(ctx, data);
    } else {
      uint8$1.ser(ctx, 211);
      bigInt64$1.ser(ctx, BigInt(data));
    }
  }
};
...
const msgpack = (encoding, single = false) => {
  const { ser: float } = single ? float32$1 : float64$1;
...
const struct = (definition) => {
  ...
  return /* @__PURE__ */ new Function(
    ...
    `[${indexes.map((i) => "s" + i)}]`,
    `[${indexes.map((i) => "d" + i)}]`,
    `return d((c,d)=>{${indexes.map((i) => `s${i}(c,d[${nameOf(keys[i])}])`).join(";")}},(c)=>{const d=${definition instanceof Array ? "[]" : "{}"};${indexes.map((i) => `d[${nameOf(keys[i])}]=d${i}(c)`).join(";")};return d})`
  )(
    define,
    keys,
...
const number = (kind, bitSize) => {
  ...
  const size2 = bitSize / 8;
  return /* @__PURE__ */ new Function(
    "d",
    `return d((c,d)=>{c.view.set${name}(c.i,d);c.i+=${size2}},(c)=>{const d=c.view.get${name}(c.i);c.i+=${size2};return d})`
  )(define);
};
